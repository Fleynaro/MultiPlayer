...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
	...
	if (aChar == '\n')
	{
		...
		int nextLine = coord.mLine + 1;
		if (cindex > 2 && cindex < line.size())
		{
			auto glyph1 = *(line.begin() + cindex - 1);
			auto glyph2 = *(line.begin() + cindex);
			if (glyph1.mChar == '{' && glyph2.mChar == '}'
				|| glyph1.mChar == '(' && glyph2.mChar == ')'
				|| glyph1.mChar == '[' && glyph2.mChar == ']'
				|| glyph1.mChar == '<' && glyph2.mChar == '>') {
				InsertLine(nextLine);
				nextLine++;
		...
		auto debug_lineSize = line.size();
		auto debug_mLine = coord.mLine;
		auto debug_cindex = cindex;

		InsertLine(nextLine);
		auto& newLine = mLines[nextLine];

		size_t whitespaceSize = 0;
		size_t whitespaceSize_newLine = 0;
		if (mLanguageDefinition.mAutoIndentation)
		{
			for (size_t it = 0; it < line.size() && isascii(line[it].mChar) && isblank(line[it].mChar); ++it) {
				newLine.push_back(line[it]);
			}
			whitespaceSize_newLine = newLine.size();

			if (nextLine != coord.mLine + 1) {
				auto& middleLine = mLines[coord.mLine + 1];
				middleLine.insert(middleLine.end(), newLine.begin(), newLine.end());
				middleLine.push_back(Glyph('\t', PaletteIndex::Default));
				whitespaceSize = middleLine.size();
				u.mAdded = "\n\n";

			}
			else {
				whitespaceSize = whitespaceSize_newLine;
				u.mAdded = "\n";
			}
		}
		...
		u.mAddedEnd = Coordinates(nextLine, GetCharacterColumn(nextLine, (int)whitespaceSize_newLine));
	...
	else
	{
		...
		if (e > 0)
		{
			...
			bool hasGlyphAC = false;
			char newBuf[20];
			AutoComplete(buf, newBuf, hasGlyphAC);

			for (auto p = newBuf; *p != '\0'; p++, ++cindex) {
				line.insert(line.begin() + cindex, Glyph(*p, PaletteIndex::Default));
			}
			u.mAdded = newBuf;

			auto newCursorPos = Coordinates(coord.mLine, GetCharacterColumn(coord.mLine, cindex));
			u.mAddedEnd = newCursorPos;
			newCursorPos.mColumn -= (int)hasGlyphAC;
...
void TextEditor::AutoComplete(const char* buf, char* newBuf, bool& hasGlyphAC)
{
	int cindex = 0;
	...
	for (auto p = buf; *p != '\0'; p++, ++cindex) {
		...
		//if there are any of these glyphs (as last glyph!)
		if (pre_cindex != cindex) {
			hasGlyphAC = true;
		} else
			hasGlyphAC = false;
	}
