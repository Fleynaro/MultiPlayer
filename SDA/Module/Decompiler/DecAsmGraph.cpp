...
void ShowCode(LinearView::BlockList* blockList, std::map<PrimaryTree::Block*, AsmGraphBlock*>& asmBlocks, int level = 0) {
	...
	for (auto block : blockList->getBlocks()) {
		...
		printf("%s//block %s (level: %i, linearLevel: %i)\n", tabStr.c_str(), Generic::String::NumberToHex(asmBlock->ID).c_str(), decBlock->m_level, block->getLinearLevel());
	...
	if (g_SHOW_LINEAR_LEVEL_EXT) {
		levelInfo = "minLinLevel: " + std::to_string(blockList->getMinLinearLevel()) + " maxLinLevel: " + std::to_string(blockList->getMaxLinearLevel()) + "";
	}
