...
void ProgramModuleFixtureDecSamples::initSampleTest()
{
...

	{
		/*
			...
			TODO:
			1. Добавить поле с типом FunctionCallInfo в сигнатуру функции.
			При добавлении параметра/добавлении CustomStorage/изменении CallingConvention выполнить апдейт содержимого этого поля, юзая SdaHelper.h.
			Нужно сделать возможность получения Storage параметра из символа параметра функции. У нас может быть регистровый параметр, или стековый/глобальный.
...

	{
		...
		sig->addParameter("param1", findType("int32_t", ""));
		sig->addParameter("param2", findType("int32_t", ""));
		sig->addParameter("param3", findType("int32_t", ""));
		sig->setReturnType(findType("int32_t"));
	}
}
