...
def main(output: Path, pyi: Path) -> None:
    ...
    pyi_lines = ['"""Strictly typed public API exposed by the Client.dll Python module."""', "", "from typing import Final", "", "", "class Vector3:", "    x: float", "    y: float", "    z: float", "", "    def __init__(self, x: float, y: float, z: float) -> None: ...", "", "", "Entity: Final = int", "Ped: Final = int", "Vehicle: Final = int", "Object: Final = int", "Pickup: Final = int", "Blip: Final = int", "Hash: Final = int", "", "", "def wait(milliseconds: int, /) -> None: ...", "def stop_requested() -> bool: ...", ""]
    current = None
